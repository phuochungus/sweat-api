...
import { FriendRequestStatus, NotificationStatus, PostPrivacy, ReactType, UserGender } from '../src/common/enums';
...
export class SeedAll1742205341178 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const postId of postIds) {
      ...
      // Create some replies
      for (const commentId of commentIds.slice(0, faker.number.int({ min: 0, max: commentIds.length }))) {
        if (faker.datatype.boolean()) {
          const userId = faker.helpers.arrayElement(userIds);
    ...
    for (const postId of postIds) {
      ...
      for (const userId of reactingUserIds) {
        ...
        `, [
          userId,
          postId,
          faker.helpers.enumValue(ReactType)
        ]);
      }
