...
describe('Notification (e2e)', () => {
  ...
  describe('GET /user-notification', () => {
    it('should get all notifications for the authenticated user', async () => {
      const response = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${TestUtils.getMockAuthToken(testUser.id)}`)
        .expect(200);
      ...
      const foundNotification = response.body.data.find(n => n.id === testNotification.id);
  ...
  describe('PATCH /user-notification', () => {
    it('should batch update notification status', async () => {
      ...
      const updatedNotification = getResponse.body.data.find(n => n.id === testNotification.id);
