...
async function bootstrap() {
  ...
  try {
    ...
    const allUsers = await dataSource.createQueryBuilder(User, 'u').getMany();
    ...
    for (let i = 0; i < allUsers.length; i++) {
      ...
      const groupStart =
        Math.floor(i / mutualFriendGroupSize) * mutualFriendGroupSize;
      const groupEnd = Math.min(
        groupStart + mutualFriendGroupSize,
        allUsers.length,
      );
      ...
      for (let j = groupStart; j < groupEnd; j++) {
        ...
        if (!existingFriend) {
          ...
          console.log(
            `Created mutual friend relationship between user ${user.id} and user ${friend.id}`,
          );
        }
      }
    }
    ...
    for (let i = 0; i < allUsers.length; i++) {
      ...
      const targetFriendCount = Math.floor(
        (allUsers.length - 1) * friendPercentage,
      );
      ...
      const potentialFriends = allUsers.filter(
        (potentialFriend) =>
          ...
          Math.floor(potentialFriend.id / mutualFriendGroupSize) !==
            Math.floor(user.id / mutualFriendGroupSize),
      );

      // Shuffle potential friends to randomize connections
      const shuffledPotentialFriends = potentialFriends.sort(
        () => 0.5 - Math.random(),
      );

      for (
        let f = 0;
        f < Math.min(additionalFriendsNeeded, shuffledPotentialFriends.length);
        f++
      ) {
        ...
        if (!existingFriend) {
          ...
          console.log(
            `Created random friend relationship between user ${user.id} and user ${friend.id}`,
          );
        }
      }
    }
    ...
    for (let i = 0; i < allUsers.length; i++) {
      ...
      const requestCount = Math.floor(
        (allUsers.length - 1) * requestPercentage,
      );
      ...
      for (const receiver of nonFriends) {
        ...
        console.log(
          `Created friend request from user ${sender.id} to user ${receiver.id}`,
        );
      }
    }
